<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ include file ="../include/sessioncheck.jsp" %>
<%@page import="com.koreait.db.Dbconn"%>
 <%@page import="java.sql.*"%>
<%@ page import="java.sql.*,java.text.SimpleDateFormat,java.util.Date"%>
<%
	Connection conn = null;
	PreparedStatement pstmt = null;
	ResultSet rs = null;
	
	// 페이징 처리를 위한 변수 선언
	int totList = 0;
	int pagePerCount = 10;	// 페이지당 글 갯수
	int start = 0;		// mysql 시작 글번호
	int num = 0;

	
	// 페이징 처리
	String pageNum = request.getParameter("pageNum");
	if(pageNum != null && !pageNum.equals("")){
		start = (Integer.parseInt(pageNum) - 1) * pagePerCount;
	} else {
		pageNum = "1";
		start = 0;
	}
	
	try {
		conn = Dbconn.getConnection();
		if(conn != null) {
			
			// 리스트 전체 갯수 가져오기
			String sql1 = "select count(b_idx) as cnt1 from tb_board";
			pstmt = conn.prepareStatement(sql1);
			ResultSet tot_list = pstmt.executeQuery();

			if(tot_list.next()){
				int cnt1 = tot_list.getInt("cnt1");
				if(cnt1 >= 0) {
					totList = cnt1;
				}
			}
			
			
			
			
			/* String sql = "select b_idx, b_userid, b_name, b_title, b_hit, b_regdate, b_like from tb_board;" */
			// 페이징 처리할때 개수 끊어주기
			String sql = "select * from tb_board order by b_idx desc limit ?, ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, start);
			pstmt.setInt(2, pagePerCount);
			rs = pstmt.executeQuery();
		}
	}catch(Exception e){
		e.printStackTrace();
	}
%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>리스트</title>
<style>
	table {
		width: 800px;
		border: 1px solid black;
		border-collapse: collapse;
	}
	
	th, td {
		border: 1px solid black;
		padding: 10px;
		text-align: center;
	}
</style>
</head>
<body>

	<h2>리스트</h2>

	<p>총 게시물 :  <%=totList %> 개</p>

	<table>
		<tr>
			<th>번호</th>
			<th>제목</th>
			<th>글쓴이</th>
			<th>조회수</th>
			<th>날짜</th>
			<th>좋아요</th>
		</tr>
<%
		// DB에 저장되어 있는 리스트 반복문 돌려서 찍어줌
		while (rs.next()) {
			 int setList = 0; 
			String b_idx = rs.getString("b_idx");
			String b_userid = rs.getString("b_userid");
			String b_name = rs.getString("b_name");
			String b_title = rs.getString("b_title");
			String b_hit = rs.getString("b_hit");
			String b_regdate = rs.getString("b_regdate");
			String b_like = rs.getString("b_like");
			
			// 날짜계산
			SimpleDateFormat transFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			Date date = transFormat.parse(b_regdate);
			long now = System.currentTimeMillis();
			long inputDate = date.getTime();
			
			// 댓글 갯수 가져오기
			String sql = "select count(re_idx) as cnt from tb_reply where re_boardidx=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, b_idx);
			ResultSet rs_reply = pstmt.executeQuery();
			
			String replyCnt = "";
			
			if(rs_reply.next()){
				int cnt = rs_reply.getInt("cnt");
				if(cnt > 0) {
					replyCnt = "[" + cnt + "]";
				}
			}	
			
			for(int i = 1; i >= totList;  i--) {
				setList = i ;
				System.out.println(setList);
				
			}
			
%>		
		<tr>
			<td><%=totList - 10 * (Integer.parseInt(pageNum)-1)-num%></td> 
			<td><a href="view.jsp?b_idx=<%= b_idx%>"><%= b_title %></a>
<%
			// 하루 계산
			if(now - inputDate < (1000*60*60*24*1)){
%>
			<img src='./img/img.jpg' alt="새글">
<%
		}  
%>
			<%= replyCnt%>
			</td>
			<td><%= b_userid %>(<%=b_name %>)</td>
			<td><%= b_hit %></td>
			<td><%= b_regdate %></td>
			<td><%= b_like %></td>
		</tr>
		
<%
		num++;
		}

		int pageNums = 0; // 총 페이지수
		if(totList % pagePerCount == 0){
			pageNums = (totList / pagePerCount);
		}else{
			pageNums = (totList / pagePerCount) + 1;
		}
%>
<tr>
	<td colspan="6">
<%
		for(int i = 1; i <= pageNums; i++){
			out.print("<a href='list.jsp?pageNum=" + i + "'>[" + i + "]</a>&nbsp;&nbsp;&nbsp;");
		}
%>
	</td>
</tr>
	</table>
	<p><a href="write.jsp">글쓰기</a> <a href="../login.jsp">돌아가기</a></p>

</body>

</html>